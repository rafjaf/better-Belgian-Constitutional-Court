  ...
  async formatWithFallback(keys, method) {
    ...
    for await (const bundle of this.bundles) {
      ...
      if (typeof console !== "undefined") {
        ...
        // Silently ignore missing translations for editor features (not critical)
...
}
{
  ...
  var validateFileURL = function (file) {
    ...
    try {
      ...
      // Allow chrome-extension: origins to load cross-origin files
      if (HOSTED_VIEWER_ORIGINS.includes(viewerOrigin) || viewerOrigin.startsWith("chrome-extension:")) {
        return;
      }
...
function webViewerLoad() {
  ...
  try {
    ...
    // Silently ignore cross-origin access errors (expected in chrome extensions)
    // Just dispatch on our own document instead
